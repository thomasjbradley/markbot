  ...
  exec(cmd, opts, function (err, stdout) {
    ...
    if (gitOpts.allSynced) {
      if (status.remote_diff !== null) {
        listener.send('check-group:item-complete', group, allSynced, allSyncedLabel, ['There are some commits waiting to be synced']);
      } else {
    ...
    if (gitOpts.allCommitted) {
      if (!status.clean) {
        listener.send('check-group:item-complete', group, allCommitted, allCommittedLabel, ['There are some files waiting to be committed']);
      } else {
