...
const takeScreenshotAtWidth = function (listener, win, group, filePath, sizes, refScreenPath, savedPaths, cb) {
  if (sizes.length > 0) {
    let width = sizes.shift();

    listener.send('check-group:item-computing', group, filePath + '-' + width, filePath + ' — ' + width + 'px');
    ...
    setTimeout(function () {
      ...
      win.capturePage({x: 0, y:0, width: width, height: height}, function (img) {
        saveScreenshot(filePath, width, img, refScreenPath, function (path) {
          ...
          takeScreenshotAtWidth(listener, win, group, filePath, sizes, refScreenPath, savedPaths, cb);
        });
      });
    }, 600);
  } else {
...
const takeScreenshots = function (listener, win, group, file, refScreenPath, cb) {
  takeScreenshotAtWidth(listener, win, group, file.path, file.sizes.slice(0), refScreenPath, [], function (paths) {
    cb(paths);
  });
};
...
const check = function (listener, fullPath, file, group, genRefScreens) {
  ...
  onFinishLoad = function () {
    ...
    setTimeout(function () {
      takeScreenshots(listener, win, group, file, refScreenPath, function (screenshotPaths) {
        win.destroy();
        win = null;
