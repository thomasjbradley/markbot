...
const check = function (listener, fullPath, file, group) {
  let
    screenshotSizes = file.sizes.slice(),
    ...
    listenerId = function (size) { return `${file.path}-${size}`; },
    listenerLabel = function (size) { return `${file.path} — ${size}px`; },
    ipcListenerLabel = classify(`${file.path}-${Date.now()}`),
    ipcListenerResizeChannel = `__markbot-screenshots-resized-${ipcListenerLabel}`
    ;
