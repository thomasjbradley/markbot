...
const diffScreenshot = function (fullPath, group, filename, width) {
...
const check = function (fullPath, file, group) {
    ...
    takeScreenshotAtSize(win, width, height, function (img) {
      ...
      saveScreenshot(imagePath, width, img, function () {
        if (fileExists.check(getImagePath(fullPath, file.path, width, true))) {
          diffScreenshot(fullPath, group, file.path, width);
        } else {
          markbotMain.send('check-group:item-complete', group, listenerId(width), listenerLabel(width), [`Reference screenshot not found in the “${REFERENCE_SCREENSHOT_FOLDER}” folder`]);
        }
  ...
  webLoaderQueue.add(file.path, {width: MAX_WINDOW_WIDTH, height: MAX_WINDOW_HEIGHT}, function (theWindow) {
    win = theWindow;
