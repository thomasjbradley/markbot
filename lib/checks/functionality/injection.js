const __MarkbotInjectedFunctions = {
  ...
  ev: function (eventStr, opts = {}) {
    const
      defaultOpts = { bubbles: true, cancelable: true },
      allOpts = Object.assign(defaultOpts, opts)
      ;
  ...
  send: function (eventStr, opts = {}, next = null) {
    const
      defaultOpts = { type: eventStr, isTrusted: true },
      allOpts = Object.assign(defaultOpts, opts),
      win = __MarkbotInjectedFunctions.browserWindow.fromId(__MarkbotInjectedFunctions.browserWindowID)
      ;

    win.webContents.sendInputEvent(allOpts);
    if (next) window.requestAnimationFrame(next);
  },
