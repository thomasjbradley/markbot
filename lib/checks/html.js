...
module.exports.check = function (listener, filePath, file, group, matchesLock) {
  ...
  fs.readFile(fullPath, 'utf8', function (err, fileContents) {
    ...
    if (file.valid) {
      ...
      if ((file.has || file.has_not) && elementsChecker) elementsChecker.bypass();
      if (file.performance && performanceChecker) performanceChecker.bypass();

      listener.send('debug', 'Best practices, outlines, has/not & performance cannot be checked with completing validation checks.');
    }
