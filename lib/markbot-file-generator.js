...
const populateDefaults = function (markbotFile) {
  ...
  if (!markbotFile.defaults && !markbotFile.inherit) return markbotFile;
  ...
  if (markbotFile.inherit) {
    ...
    merge(newMarkbotFile, markbotFile);
  }

  if (markbotFile.defaults) {
    ...
    keys.forEach(function (key) {
      ...
      newMarkbotFile[key].forEach(function (item, i) {
        if (!newMarkbotFile.defaults[key]) return;

        newMarkbotFile[key][i] = merge(Object.assign({}, newMarkbotFile.defaults[key]), item);
      });
    });

    if (newMarkbotFile.defaults && newMarkbotFile.defaults.functionality && newMarkbotFile.html) {
      if (!newMarkbotFile.functionality) newMarkbotFile.functionality = [];

      newMarkbotFile.html.forEach(function (file) {
        newMarkbotFile.functionality.push(merge({ path: file.path }, newMarkbotFile.defaults.functionality));
      });
    }
  }
