...
const load = function (listener, url, opts, next) {
  let win;
  let speed = false;
  let networkName;
  ...
  ipcMain.on('__markbot-hidden-browser-har-generation-succeeded', function (e, har) {
    cleanup();
    next(win, har);
  });
  ...
  if (speed) {
    listener.send('debug', `Network: ${networkName}, ${speed.latency}ms⬌, ${speed.downloadThroughput}kB/s⬇, ${speed.uploadThroughput}kB/s⬆`)
